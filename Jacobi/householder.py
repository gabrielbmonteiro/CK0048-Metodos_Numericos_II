...
class metodo_house_holder:
    ...
    def metodo(self):
        ...
        return self.padronizar(A_barra), H  # usaremos H para conseguir os autovetores da matriz inicial (A), que é h*autovaloresMatrizT 
    ...
    def padronizar(self, a):
        """
        Padroniza uma matriz substituindo todos os valores muito pequenos por zero.

        Parâmetros:
        - a: Matriz (lista de listas) de números, que será padronizada.

        Retorna:
        - aux: Matriz resultante onde todos os valores absolutos menores que um pequeno limite (0.0000000001) são substituídos por zero.
        """
        
        aux = a # Ccópia da matriz de entrada para evitar modificar a matriz original
        for i in range(self.tam):
            for j in range(self.tam):
